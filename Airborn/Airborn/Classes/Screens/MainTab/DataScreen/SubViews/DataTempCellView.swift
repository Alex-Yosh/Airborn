...
struct DataTempCellView: View {
    ...
    var body: some View {
        if let temp = dataManager.latestSensorData?.temperature {
            DataScaleView(progressPercent: getPercent(temp: temp), temp: temp)
                .frame(width: 100, height: 100)
        }
    }

    func getPercent(temp: Double) -> Float {
        if temp < 16 {
            return 0.0
        } else if temp > 28 {
            return 1.0
        ...
        ZStack {
            ...
            Circle()
                ...
                .fill(AngularGradient(gradient: Gradient(stops: [
                    ...
                    .init(color: Constants.Colour.ScaleRed, location: 0.95)
                ]), center: .center))
                .rotationEffect(.degrees(54.5))
                .animation(.easeInOut, value: StartingProgress)

            VStack {
                Text(String(format: "%.1f", temp) + Constants.dataTypes.temperature.metric)
